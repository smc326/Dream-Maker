...
from .core.client import DreamClient, TokenHolder
...
class HassTokenHolder(TokenHolder, ABC):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry):
        ...
        self._store = Store(hass, 1, 'Dream/{}.token'.format(account_cfg.username))
        _LOGGER.debug('token初始化{}'.format(self._store))
    ...
    async def async_get(self) -> (str, datetime):
        try:
            ...
            _LOGGER.debug('已从HomeAssistant加载到缓存的token{}'.format(data))
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    account_cfg = AccountConfig(hass, entry)
    client = DreamClient(account_cfg.username, account_cfg.password, HassTokenHolder(hass, entry))
...
async def get_available_devices(client: DreamClient) -> List[DreamDevice]:
    # 从账号中获取所有设备
...
async def get_virtual_devices(client: DreamClient) -> List[DreamDevice]:
    ...
    for file in glob.glob(target_folder + '/*.json'):
        with open(file, 'r') as fp:
            device = json.load(fp)
            device['virtual'] = True
            device = DreamDevice(client, device)
