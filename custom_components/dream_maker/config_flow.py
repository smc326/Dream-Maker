...
from .core.client import DreamClient, DreamClientException
from .core.config import AccountConfig, DeviceFilterConfig, EntityFilterConfig

_LOGGER = logging.getLogger(__name__)


class DreamConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    VERSION = 2
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                # 校验账号密码是否正确
                client = DreamClient(user_input[CONF_USERNAME], user_input[CONF_PASSWORD])
                await client.try_login()

                return self.async_create_entry(title="造梦者 - {}".format(user_input[CONF_USERNAME]), data={
                    'account': user_input
                })
            except DreamClientException as e:
